...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": ["https://localhost:5173",  "https://172.20.10.2:5173", "https://192.168.43.159:5173"]}})
final_perclos_score = None 

# camera matrix obtained from the camera calibration script
camera_matrix = np.array(
    [[899.12150372, 0., 644.26261492],
     [0., 899.45280671, 372.28009436],
     [0, 0,  1]], dtype="double")

# distortion coefficients obtained from the camera calibration script
dist_coeffs = np.array(
    [[-0.03792548, 0.09233237, 0.00419088, 0.00317323, -0.15804257]], dtype="double")

def _get_landmarks(lms):
    # Function to get the biggest face landmarks
    surface = 0
    for lms0 in lms:
...
def gen():
    ...
    parser.add_argument('--show_fps', type=bool, default=True, metavar='', help='Show the actual FPS of the capture stream, default is true')
    parser.add_argument('--show_proc_time', type=bool, default=True, metavar='', help='Show the processing time for a single frame, default is true')
    parser.add_argument('--show_eye_proc', type=bool, default=False, metavar='', help='Show the eyes processing, deafult is false')
    parser.add_argument('--show_axis', type=bool, default=True, metavar='', help='Show the head pose axis, default is true')
    parser.add_argument('--verbose', type=bool, default=False, metavar='', help='Prints additional info, default is false')
    parser.add_argument('--smooth_factor', type=float, default=0.5, metavar='', help='Sets the smooth factor for the head pose estimation keypoint smoothing, default is 0.5')
    ...
    # Instantiate the mediapipe face mesh model
    ...
    ip_webcam_url = 'https://192.168.43.49:8080/video'
    ...
    # Set the duration for evaluation (60 seconds)
    evaluation_duration = 20  # seconds
    evaluation_start_time = time.perf_counter()
    perclos_score = None

    while (time.perf_counter() - evaluation_start_time) < evaluation_duration:
        ...
        if lms:
            ...
            boundary_box = (int(width / 8), int(height / 8), int(width * 7 / 8), int(height * 7 / 8))
            cv2.rectangle(frame, (boundary_box[0], boundary_box[1]), (boundary_box[2], boundary_box[3]), (0, 255, 0), 2)
            # Check if the biggest face landmarks are within the boundary box
            if not all(boundary_box[0] < p[0] < boundary_box[2] and boundary_box[1] < p[1] < boundary_box[3] for p in landmarks):
                cv2.putText(frame, "Please move closer and fill 3/4 of the frame", (10, 200),
        ...
        global final_perclos_score  
        final_perclos_score = perclos_score
    if final_perclos_score is not None: 
        print(f"Final PERCLOS Score: {final_perclos_score}")
    print("Evaluation completed (60 seconds).")
...
@app.route('/perclos_score', methods=['GET'])
def get_perclos_score():
    global final_perclos_score  # Access the global score
    if final_perclos_score is not None:
        print(final_perclos_score)
        return str(final_perclos_score)
    else:
